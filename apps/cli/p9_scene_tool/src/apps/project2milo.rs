...
use grim::scene::{Object, PackedObject, PropAnim, PropKeys};
...
fn load_midi(project_dir: &Path, is_gdrb: bool) -> Option<Object> {
    ...
    if let Some(track) = venue_track {
        let mut keys = load_venue_track(track);
    ...
    for (char_track_name, long_name) in char_track_names.iter() {
        ...
        if let Some(track) = char_track {
            let mut keys = load_char_track(track, long_name);
            prop_keys.append(&mut keys);
...
fn load_venue_track(track: &MidiTrack) -> Vec<PropKeys> {
    //let mut prop_keys = HashMap::new();
...
fn load_char_track(track: &MidiTrack, char_name: &str) -> Vec<PropKeys> {
    let mut prop_keys = Vec::new();
